using System.Net;
using Invoicing.Domain.Entities;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Logging;

namespace Invoicing.Infrastructure.Persistence;

public class ApplicationDbContextInitialiser
{
    private readonly ILogger<ApplicationDbContextInitialiser> _logger;
    private readonly ApplicationDbContext _context;

    public ApplicationDbContextInitialiser(ILogger<ApplicationDbContextInitialiser> logger,
        ApplicationDbContext context)
    {
        _logger = logger;
        _context = context;
    }

    public async Task InitialiseAsync()
    {
        try
        {
            if (_context.Database.IsNpgsql())
            {
                await _context.Database.MigrateAsync();
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An error occurred while initialising the database.");
            throw;
        }
    }

    public async Task SeedAsync()
    {
        try
        {
            await TrySeedAsync();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An error occurred while seeding the database.");
            throw;
        }
    }

    public async Task TrySeedAsync()
    {
        if (!_context.Invoices.Any())
        {
            var invoices = new List<Invoice>
            {
                new Invoice
                {
                    Number = 1,
                    DueDate = DateTime.UtcNow.AddDays(7),
                    Currency = Currency.USD,
                    Notes =
                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin ipsum ipsum, vehicula in semper sed, lacinia in quam. Pellentesque mollis, justo feugiat laoreet hendrerit, dui.",
                    Type = InvoiceType.Current,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Nullam sollicitudin",
                            Price = 120,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 120
                        },
                        new InvoiceItem
                        {
                            Description = "Quisque faucibus",
                            Price = 85,
                            Metric = Metric.Months,
                            Count = 1,
                            Subtotal = 85
                        },
                    },
                    SellerId = 1,
                    BuyerId = 2,
                    // Insert BuyerEmail if you want to test bill emailing
                    Total = 205,
                    RegularInvoiceDate = DateTime.UtcNow,
                    IsRegular = true,
                },
                new Invoice
                {
                    Number = 2,
                    DueDate = DateTime.UtcNow.AddDays(7),
                    Currency = Currency.USD,
                    Notes =
                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin ipsum ipsum, vehicula in semper sed, lacinia in quam. Pellentesque mollis, justo feugiat laoreet hendrerit, dui.",
                    Type = InvoiceType.Current,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Nullam sollicitudin",
                            Price = 120,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 120
                        },
                        new InvoiceItem
                        {
                            Description = "Quisque faucibus",
                            Price = 85,
                            Metric = Metric.Months,
                            Count = 1,
                            Subtotal = 85
                        },
                    },
                    SellerId = 2,
                    BuyerId = 1,
                    // Insert BuyerEmail if you want to test bill emailing
                    Total = 504,
                },
                new Invoice
                {
                    Number = 3,
                    DueDate = DateTime.UtcNow.AddDays(10),
                    Currency = Currency.USD,
                    Notes =
                        "In tortor augue, suscipit in diam vitae, tempor pellentesque mauris. Morbi vel elit porttitor, imperdiet sem eu, eleifend massa. Nulla maximus enim diam, vel faucibus.",
                    Type = InvoiceType.Current,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Ut nulla mauris",
                            Price = 90,
                            Metric = Metric.Quantity,
                            Count = 3,
                            Subtotal = 270
                        },
                        new InvoiceItem
                        {
                            Description = "Nam ornare ipsum",
                            Price = 110,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 110
                        },
                        new InvoiceItem
                        {
                            Description = "Suspendisse in placerat",
                            Price = 200,
                            Metric = Metric.Quantity,
                            Count = 3,
                            Subtotal = 600
                        },
                    },
                    SellerId = 2,
                    BuyerId = 1,
                    Total = 980,
                },
                new Invoice
                {
                    Number = 4,
                    DueDate = DateTime.UtcNow.AddDays(12),
                    Currency = Currency.USD,
                    Notes =
                        "Nullam sagittis ex nulla, iaculis accumsan est malesuada facilisis. Vivamus malesuada malesuada mauris, mollis lobortis massa. Nunc aliquam accumsan libero. Donec tincidunt quam non mollis.",
                    Type = InvoiceType.Current,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Class aptent",
                            Price = 45,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 45
                        },
                        new InvoiceItem
                        {
                            Description = "Cras vel",
                            Price = 60,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 60
                        },
                        new InvoiceItem
                        {
                            Description = "Vivamus tincidunt",
                            Price = 112,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 112
                        },
                    },
                    SellerId = 2,
                    BuyerId = 3,
                    Total = 217,
                },
                new Invoice
                {
                    Number = 5,
                    DueDate = DateTime.UtcNow.AddDays(15),
                    Currency = Currency.USD,
                    Notes =
                        "Cras blandit lacus lacus, in molestie est ultrices ac. Phasellus eleifend dapibus libero, eget hendrerit risus. Nulla posuere sapien nec justo consectetur pharetra. Proin id.",
                    Type = InvoiceType.Current,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Morbi a mattis",
                            Price = 55,
                            Metric = Metric.Months,
                            Count = 1,
                            Subtotal = 55
                        },
                        new InvoiceItem
                        {
                            Description = "Sed tincidunt congue",
                            Price = 162,
                            Metric = Metric.Months,
                            Count = 1,
                            Subtotal = 162
                        },
                        new InvoiceItem
                        {
                            Description = "Praesent finibus",
                            Price = 105,
                            Metric = Metric.Months,
                            Count = 1,
                            Subtotal = 105
                        },
                    },
                    SellerId = 3,
                    BuyerId = 1,
                    Total = 322,
                },
                new Invoice
                {
                    Number = 6,
                    DueDate = DateTime.UtcNow.AddDays(20),
                    Notes = "Proin mollis eros et ornare dignissim.",
                    Currency = Currency.USD,
                    Type = InvoiceType.Current,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Nunc variuc",
                            Price = 90,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 90
                        },
                        new InvoiceItem
                        {
                            Description = "Nam elit",
                            Price = 75,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 75
                        },
                        new InvoiceItem
                        {
                            Description = "Nulla pharetra",
                            Price = 140,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 140
                        },
                    },
                    SellerId = 4,
                    BuyerId = 1,
                    Total = 305,
                },
                new Invoice
                {
                    Number =7,
                    DueDate = DateTime.UtcNow.AddDays(20),
                    Notes = "Proin mollis eros et ornare dignissim.",
                    Currency = Currency.USD,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Nunc variuc",
                            Price = 90,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 90
                        },
                        new InvoiceItem
                        {
                            Description = "Nam elit",
                            Price = 75,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 75
                        },
                        new InvoiceItem
                        {
                            Description = "Nulla pharetra",
                            Price = 140,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 140
                        },
                    },
                    SellerId = 2,
                    Total = 305,
                    Type = InvoiceType.Archived,
                },
                new Invoice
                {
                    Number = 8,
                    DueDate = DateTime.UtcNow.AddDays(12),
                    Notes = "Proin mollis eros et ornare dignissim.",
                    Currency = Currency.USD,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Nunc variuc",
                            Price = 90,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 90
                        },
                        new InvoiceItem
                        {
                            Description = "Nam elit",
                            Price = 75,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 75
                        },
                        new InvoiceItem
                        {
                            Description = "Nulla pharetra",
                            Price = 140,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 140
                        },
                    },
                    SellerId = 5,
                    BuyerId = 1,
                    Total = 305,
                },
                new Invoice
                {
                    Number = 9,
                    DueDate = DateTime.UtcNow.AddDays(1).AddHours(2),
                    Notes = "Proin mollis eros et ornare dignissim.",
                    Currency = Currency.USD,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Nunc variuc",
                            Price = 90,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 90
                        },
                        new InvoiceItem
                        {
                            Description = "Nam elit",
                            Price = 75,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 75
                        },
                        new InvoiceItem
                        {
                            Description = "Nulla pharetra",
                            Price = 140,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 140
                        },
                    },
                    SellerId = 2,
                    Total = 305,
                },
                new Invoice
                {
                    Number = 10,
                    DueDate = DateTime.UtcNow.AddDays(15),
                    Currency = Currency.USD,
                    Notes =
                        "Cras fringilla porttitor mauris ut auctor. Vivamus velit magna, consequat ut fermentum consectetur, congue in leo. Donec ut facilisis felis.",
                    Type = InvoiceType.Current,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Mauris venenatis",
                            Price = 80,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 80
                        },
                        new InvoiceItem
                        {
                            Description = "Nulla nibh",
                            Price = 215,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 215
                        },
                    },
                    SellerId = 3,
                    Total = 295,
                },
            };
            var lastNumber = (int)invoices.Max(inv => inv.Number) + 1;
            for (int i = lastNumber; i < lastNumber+15; i++)
            {
                var invoice = new Invoice
                {
                    Number = i,
                    DueDate = DateTime.UtcNow.AddDays(Random.Shared.Next(1,25)),
                    Currency = Currency.USD,
                    Notes =
                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin ipsum ipsum, vehicula in semper sed, lacinia in quam. Pellentesque mollis, justo feugiat laoreet hendrerit, dui.",
                    Type = InvoiceType.Current,
                    ApprovalStatus = Random.Shared.Next(0,2)==1 ? ApprovalStatus.Pending : ApprovalStatus.Approved,
                    Items = new List<InvoiceItem>
                    {
                        new InvoiceItem
                        {
                            Description = "Nullam sollicitudin",
                            Price = 120,
                            Metric = Metric.Quantity,
                            Count = 1,
                            Subtotal = 120
                        },
                        new InvoiceItem
                        {
                            Description = "Quisque faucibus",
                            Price = 85,
                            Metric = Metric.Months,
                            Count = 1,
                            Subtotal = 85
                        },
                    },
                    SellerId = 1,
                    BuyerId = Random.Shared.Next(2,6),
                    // Insert BuyerEmail if you want to test bill emailing
                    Total = Random.Shared.NextInt64(200,500),
                };
                invoices.Add(invoice);
            }
            _context.Invoices.AddRange(invoices);
            await _context.SaveChangesAsync();
        }

        //if (!_context.Bills.Any())
        //{
        //    var bills = new List<Bill>
        //    {
        //        new Bill {Status = BillStatus.Paid, InvoiceId = 4, PaymentMethodId = 3}
        //    };

        //    _context.Bills.AddRange(bills);
        //    await _context.SaveChangesAsync();
        //}

        if (!_context.Bills.Any())
        {
            var bills = new List<Bill> {    
                new Bill {
                    Status = BillStatus.Unpaid,
                    InvoiceId = 1,
                    PaymentMethodId = 5,
                    ApprovalStatus = ApprovalStatus.None
                },
                new Bill {
                    Status = BillStatus.Unpaid,
                    InvoiceId = 2,
                    PaymentMethodId = 5,
                    ApprovalStatus = ApprovalStatus.None
                },
                new Bill {
                    Status = BillStatus.Unpaid,
                    InvoiceId = 3,
                    PaymentMethodId = 6,
                    ApprovalStatus = ApprovalStatus.None
                },
                new Bill {
                    Status = BillStatus.Unpaid,
                    InvoiceId = 4,
                    PaymentMethodId = 8,
                    ApprovalStatus = ApprovalStatus.None
                },
                new Bill {
                    Status = BillStatus.Unpaid,
                    InvoiceId = 5,
                    PaymentMethodId = 9,
                    ApprovalStatus = ApprovalStatus.None
                },
                new Bill {
                    Status = BillStatus.Paid,
                    InvoiceId = 6,
                    PaymentMethodId = 3,
                    ApprovalStatus = ApprovalStatus.Approved
                }
            };

            _context.Bills.AddRange(bills);
            await _context.SaveChangesAsync();
        }

        //if (!_context.Companies.Any())
        //{
        //    var companies = new List<Company> {
        //            new Company {
        //                Name = "Figma",
        //                Logo = Convert.FromBase64String(""),
        //                PaymentMethods =  new List<PaymentMethod>()
        //                {
        //                    new PaymentMethod()
        //                    {
        //                        Title = "**** **** **** 1111",
        //                        Additional = "Expires 02/28",
        //                        MethodType = PaymentMethodType.Card,
        //                    },
        //                    new PaymentMethod()
        //                    {
        //                        Title = "**** **** **** 1234",
        //                        Additional = "Expires 05/27",
        //                        MethodType = PaymentMethodType.Card,
        //                    },
        //                    new PaymentMethod()
        //                    {
        //                        Title = "Routing number (ACH)",
        //                        Additional = "Account number",
        //                        MethodType = PaymentMethodType.Bank,
        //                    },
        //                    new PaymentMethod()
        //                    {
        //                        Title = "email@gmail.com",
        //                        MethodType = PaymentMethodType.Paypal,
        //                    }
        //                }
        //            },
        //            new Company {
        //                Name = "Amazon",
        //                Logo = Convert.FromBase64String(""),
        //                PaymentMethods = new List<PaymentMethod> {
        //                    new PaymentMethod {
        //                        Title = "email@gmail.com",
        //                        MethodType = PaymentMethodType.Paypal,
        //                    }
        //                }
        //            },
        //            new Company {
        //                Name = "UpWork",
        //                Logo = Convert.FromBase64String(""),
        //                PaymentMethods = new List<PaymentMethod> {
        //                    new PaymentMethod {
        //                        Title = "email@gmail.com",
        //                        MethodType = PaymentMethodType.Paypal,
        //                    },
        //                    new PaymentMethod {
        //                        Title = "email@gmail.com",
        //                        MethodType = PaymentMethodType.Paypal,
        //                    }
        //                }
        //            },
        //            new Company {
        //                Name = "Youtube",
        //                Logo = Convert.FromBase64String(""),
        //                PaymentMethods = new List<PaymentMethod> {
        //                    new PaymentMethod {
        //                        Title = "email@gmail.com",
        //                        MethodType = PaymentMethodType.Paypal,
        //                    }
        //                }
        //            },
        //            new Company {
        //                Name = "ABC Company",
        //                Logo = Convert.FromBase64String(""),
        //                PaymentMethods = new List<PaymentMethod> {
        //                    new PaymentMethod {
        //                        Title = "email@gmail.com",
        //                        MethodType = PaymentMethodType.Paypal,
        //                    }
        //                }
        //            }
        //        };
        //    _context.AddRange(companies);
        //    await _context.SaveChangesAsync();
        //}
    }
}